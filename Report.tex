...
Hardware coherence mechanisms for large-scalemachines are complex and costly, but existing software mechanisms for message-passing machines have not provided a performance-competitive solution.\\
In  [CIT. Multiprocessor cache coherence based on virtual memory support  petersen, Li 1995]  are presented the VM-based cache coherence that ensures that accesses to the same virtual address are kept coherent. This works at virtual memory pages granularity. 
